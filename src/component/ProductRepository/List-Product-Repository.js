...
class ListProductRepository extends Component {
    ...
    componentDidMount(){
        ...
        if(match && match.params){
            ...
            this.apiService.getListProductRepository(page, 10, this.state.categoryID, this.props.access_token).then((data)=>{
                this.setState({
                            ...
                            <div className="custom-pagination">
                                <nav aria-label="Page navigation example">
                                    <ul className="pagination">
                                        {+this.state.Page > 1 ?
                                        <li className="page-item"><Link to={`/ProductRepository/${(+this.state.Page - 1)}`}  onClick={this.handlePreviousPage}>← 10 Trước</Link></li>
                                        : null
                                        }
                                        {this.state.Page < this.state.TotalPage ?
                                        <li className="page-item"><Link to={`/ProductRepository/${(+this.state.Page + 1)}`} onClick={this.handleNextPage}> Sau 10 →</Link></li>
                                        : null
                                        }
                                    </ul>
                                </nav>
...
export default withRouter(connect(mapStateToProps) (ListProductRepository));
